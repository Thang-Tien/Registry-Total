								...
								(err, same) => {
									if (same) {
										...
										bcrypt.hash(
											...
											(err, hash) => {
												connection.query(
													`UPDATE users SET password = "${hash}", last_change_password_date = NOW() WHERE user_id = ${req.user.user_id}`,
													(err, result, field) => {
														if (err) {
															return res
																.status(500)
	...
	connection.query(
		...
		(err, result, fields) => {
			...
			bcrypt.hash(otp.toString(), 12, (err, hashedOTP) => {
				...
				connection.query(
					`UPDATE users SET reset_password_otp = "${hashedOTP}", otp_expired_date = NOW() + INTERVAL 3 HOUR WHERE email = "${req.body.email}"`,
					(err, result, fields) => {
						if (err) {
							return res.status(500).json({
								status: "Failed",
							...
							sendEmail(
								{
									...
									subject:
										"Password reset otp (only valid for 3 hours)",
									text: otp.toString(),
								},
								(err, info) => {
									if (err) {
...
exports.resetPassword = async (req, res) => {
	connection.query(
		`SELECT *, NOW() as "current_time" FROM users WHERE email = "${req.body.email}"`,
		(err, result, fields) => {
			if (err) {
				return res.status(500).json({
					status: "Failed",
					...
					if (!same) {
						...
						bcrypt.hash(
							...
							(err, encrypted) => {
								...
								connection.query(
									`UPDATE users SET password = "${encrypted}" WHERE user_id = ${result[0].user_id}`,
									(err, result, fields) => {
										if (err) {
											return res.status(500).json({
												status: "Failed",
