...
exports.countTotalAboutToExpiredInspectionsOfAllCentre = async (req, res) => {
    connection.query(
        `SELECT * FROM inspections WHERE expired_date >= DATE_SUB(NOW(), INTERVAL 1 MONTH) AND expired_date <= NOW()`,
        (err, result, fields) => {
            if (err) {
                return res.status(500).json({
                    status: "Failed",
...
exports.createInspection = (req, res) => {
    const user_id = req.body.user_id; // You should include userID in the request body
    const centre_id = req.body.centre_id; // You should include centreID in the request body
