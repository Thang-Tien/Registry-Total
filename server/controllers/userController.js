...
exports.getUser = (req, res) => {
    ...
    connection.query(`SELECT * FROM users WHERE ${queryString ? queryString : 1}`, utils.getQueryValue(req.query), (err, result, fields) => {
        if (err) {
...
exports.createUser = (req, res) => {
    connection.query(`SELECT * FROM users WHERE ssn = ? OR phone = ? OR email = "?"`,
        ...
        (err, result, fields) => {
            ...
            if (result.length != 0) {
                ...
                bcrypt.hash(password.toString(), 12, (err, hashedPassword) => {
                    connection.query(`INSERT INTO users (ssn, date_of_birth, name, phone, email, password, centre_id, role, active) VALUE(?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                        [req.body.ssn, formatedDOB, req.body.name, req.body.phone, req.body.email, hashedPassword, req.body.centre_id, "staff", true],
                        (err, result, fields) => {
                            if (err) {
                                return res.status(500).json({
                                    status: "Failed",
...
exports.deleteUser = (req, res) => {
    connection.query(`SELECT * FROM users WHERE user_id = ${req.body.user_id}`, (err, result, fields) => {
        if (err) {
            return res.status(500).json({
                status: "Failed",
                error: err
