...
const getProvinceNames = () => {
    let ret = [];
    pcvn.getProvinces().forEach((province) => {
...
const getAreasOfSide = (side) => {
    if (side === "Miền Bắc")
        return ["Tây Bắc Bộ", "Đông Bắc Bộ", "Đồng bằng Sông Hồng"];
    if (side === "Miền Trung")
        return ["Bắc Trung Bộ", "Duyên hải Nam Trung Bộ", "Tây Nguyên"];
